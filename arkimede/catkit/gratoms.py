...
class Gratoms(ase.Atoms):
    ...
    def connectivity(self):
        try: connectivity = nx.to_numpy_matrix(self._graph)
        except: connectivity = nx.to_numpy_array(self._graph)
    ...
    def __getitem__(self, i):
        ...
        for con in copy.deepcopy(self.constraints):
            if isinstance(
                con, (ase.constraints.FixConstraint, ase.constraints.FixBondLengths)
            ):
                try:
    ...
    def __iadd__(self, other):
        ...
        if isinstance(other, Gratoms):
            if isinstance(self._graph, nx.MultiGraph) & isinstance(
                other._graph, nx.Graph
            ):
