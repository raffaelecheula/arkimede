...
from .symmetry import Symmetry, get_standardized_cell
from . import utils
...
class SlabGenerator(object):
    ...
    def __init__(
        ...
        self.delta_ncoord = delta_ncoord
        ...
        if standardize_bulk:
            bulk = get_standardized_cell(bulk, tol=1e-2)
        else:
            warnings.warn(
                (
        ...
        if primitive:
            primitive_bulk = get_standardized_cell(
    ...
    def get_unique_terminations(self):
        ...
        if len(zcoords) > 1:
            ...
            sym = Symmetry(self._bulk, self.tol)
    ...
    def get_slab_basis(self, iterm=0, maxn=20):
        ...
        #surf_atoms = diff != 0
        surf_atoms = diff > self.delta_ncoord
    ...
    def get_slab(self, size=1, iterm=0, orthogonal=False):
        ...
        slab = self.set_size(slab, size, orthogonal=orthogonal)
    ...
    def set_size(self, slab, size, orthogonal=False):
        ...
        if isinstance(size, (int, np.integer)):
            ...
            if metrics:
                ...
                matrix = sorted(metrics, key=lambda x: (x[order[0]], x[order[1]]))[0][
                    -1
                ]
...
def generate_indices(max_index):
    ...
    unique_index = index.T[np.where(utils.list_gcd(index) == 1)]
